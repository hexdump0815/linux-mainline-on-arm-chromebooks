...
- it is possible to use the flashrom command to also read and write the main application (AP) and embedded controller (EC) firmware remotely via suzyqable. important for that is that the flashrom binary has to support that mode (i.e. special chromeos build of flashrom or more recommended by using another cr50 enabled chromebook which should have a useable flashrom command by default). the magic for this is the "-p raiden_debug_spi:target=AP"  or "-p raiden_debug_spi:target=EC" command line option to the flashrom command. more info can be found here: https://wiki.mrchromebox.tech/Unbricking#Unbricking.2FFlashing_with_a_Suzy-Q_cable - here: https://chromium.googlesource.com/chromiumos/platform/ec/+/cr50_stab/docs/case_closed_debugging.md#Flash-AP and here: https://chromium.googlesource.com/chromiumos/platform/ec/+/cr50_stab/docs/case_closed_debugging.md#Flash-EC ... this remote flashing can also be useful in cases where it is not possible to enable developer mode for a chromebook with a broken keyboard - in this case one can just remote read the AP firmware, adjust the gbb flags using the futility command on the obtained file and then flashing it back remotely (as usual all this is done on ones own risk and one should know and understand what the steps are doing - it worked well for me in such a case)
